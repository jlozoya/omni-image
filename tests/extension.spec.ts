...
const test = base.extend<{ context: BrowserContext; editor: Page; extensionId: string }>({
  context: async ({}, use, testInfo) => {
    ...
    const context = await chromium.launchPersistentContext(resolve(root, 'profile'), {
      ...
      downloadsPath: resolve(root, 'downloads'),
      args: [
...
const sample = {
  ...
  buffer: Buffer.from(
    '<svg xmlns="http://www.w3.org/2000/svg" width="200" height="100"><path fill="red" d="M0 0h100v100H0z"/><path fill="blue" d="M100 0h100v100H100z"/></svg>',
  ),
};
...
async function pixel(page: Page, x: number, y: number) {
  return page
    .locator('canvas[aria-label="Resultado editado"]')
    .evaluate(
      (canvas, p) => Array.from((canvas as HTMLCanvasElement).getContext('2d')!.getImageData(p.x, p.y, 1, 1).data),
      { x, y },
    );
}
...
test('placement at unchanged dimensions, undo, rotation and saved draft', async ({ editor }) => {
  ...
  await expect
    .poll(() =>
      editor
        .locator('canvas[aria-label="Imagen para recortar"]')
...
test('annotations are flattened and survive reload', async ({ editor }) => {
  ...
  await editor.mouse.move(bounds.x + bounds.width * 0.1, bounds.y + bounds.height * 0.1);
  await editor.mouse.down();
  await editor.mouse.move(bounds.x + bounds.width * 0.4, bounds.y + bounds.height * 0.7, { steps: 4 });
...
test('clipboard, partial batch failure and ZIP', async ({ editor }) => {
  ...
  await editor
    .getByLabel('Agregar imágenes', { exact: true })
    .setInputFiles({ name: 'broken.png', mimeType: 'image/png', buffer: Buffer.from('broken image') });
  ...
  await add(editor, [
    {
      ...
      mimeType: 'image/svg+xml',
      buffer: Buffer.from(
        '<svg xmlns="http://www.w3.org/2000/svg" width="800" height="180"><path fill="white" d="M0 0h800v180H0z"/><text x="35" y="110" font-family="Arial" font-size="64" fill="black">HELLO OMNI 123</text></svg>',
      ),
    },
  ]);
...
test('full-page capture stitches pixels, restores scroll, and opens editor', async ({ context, editor }) => {
  const server = createServer((_request, response) => {
    ...
    response.end(
      '<!doctype html><style>html,body{margin:0}body{height:2100px;background:linear-gradient(red,blue)}header{position:fixed;top:0;background:white;height:30px;width:100%}</style><header>Fixed header</header><p>Capture fixture</p>',
    );
  });
  ...
  try {
    ...
    await editor.evaluate(async () => {
      await (globalThis as any).chrome.storage.local.set({ captureSettings: { destination: 'editor', format: 'png' } });
    });
    await webpage.bringToFront();
    const opened = context.waitForEvent('page');
    await editor.evaluate(async () => {
      await (globalThis as any).chrome.runtime.sendMessage({ type: 'START_FULL_CAPTURE' });
    });
    ...
    const size = await captureEditor
      .locator('canvas[aria-label="Resultado editado"]')
...
test('export downloads real output', async ({ editor }) => {
  const png = await editor.evaluate(() => {
    ...
    canvas.height = 300;
    const ctx = canvas.getContext('2d')!,
      pixels = ctx.createImageData(400, 300);
    let seed = 12;
...
test('every output format encodes through the shared pipeline', async ({ editor }) => {
  ...
  for (const format of [
    ...
    await editor.getByRole('button', { name: 'Descargar imagen actual', exact: true }).click();
    await expect(editor.locator('.editor-section .inline-status'), format).toContainText('Descarga iniciada:', {
      timeout: 30000,
    });
...
test('region capture uses selected pixels; Escape restores a full-page capture', async ({ context, editor }) => {
  ...
  try {
    const webpage = await context.newPage();
    await webpage.goto(`http://127.0.0.1:${(server.address() as { port: number }).port}`);
    await editor.evaluate(() =>
      (globalThis as any).chrome.storage.local.set({ captureSettings: { destination: 'editor', format: 'webp' } }),
    );
    await webpage.bringToFront();
    ...
    const opened = context.waitForEvent('page');
    await webpage.mouse.move(100, 100);
    await webpage.mouse.down();
    await webpage.mouse.move(300, 250, { steps: 5 });
    ...
    await expect
      .poll(() =>
        editor.evaluate(
          async () => (await (globalThis as any).chrome.storage.local.get('captureStatus')).captureStatus.message,
        ),
      )
      .toBe('Captura abierta en el editor.');
    await webpage.bringToFront();
    await webpage.evaluate(() => scrollTo(0, 400));
    ...
    await expect
      .poll(() =>
        editor.evaluate(
          async () => (await (globalThis as any).chrome.storage.local.get('captureStatus')).captureStatus.ok,
        ),
      )
      .toBe(false);
  } finally {
