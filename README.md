...
1. Drag the crop box or its corner handles, or enter numeric coordinates. The frame may reach outside the image, which adds canvas around it: the surplus is filled with the background colour, or left transparent when the output format keeps alpha. Crop mode shows the image inside a workspace with room to drag the frame past its edges. Fixed ratios include 1:1, 4:3, 16:9 and 9:16. Rotate in 90-degree steps or flip either axis.
2. **Tamaño del lienzo** sets the output area and fits the whole image inside it - nothing is cropped, and the surplus takes the background colour. It releases the aspect lock and turns manual placement on, since that is what contains the image rather than stretching it. **Ajustar lienzo a la imagen** returns the canvas to the image's own size. The scale control pairs its slider with a typable number and takes the mouse wheel (1% a notch, 10% with Shift) without scrolling the sidebar. A zero dimension is automatic; with **Mantener proporción**, entering one dimension calculates the other. Disable the lock and enable manual placement to position/scale the source inside an output canvas, even when its dimensions match the original.
