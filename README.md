...
2. Set output dimensions. A zero dimension is automatic; with **Mantener proporción**, entering one dimension calculates the other. Disable the lock and enable manual placement to position/scale the source inside an output canvas, even when its dimensions match the original. The placement panel stays available under the crop tool so the image can be refitted right after the crop area changes; the crop canvas shows the original with its frame, so the scale reads on the edited result.
