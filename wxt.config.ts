...
export default defineConfig({
  ...
  hooks: {
    'build:publicAssets': async (wxt, files) => {
      ...
      files.push({
        absoluteSrc: resolve(root, 'node_modules/tesseract.js/dist/worker.min.js'),
      ...
      for (const name of await readdir(coreDir)) {
        if (name.endsWith('-lstm.wasm.js') || name.endsWith('-lstm.wasm'))
          files.push({ absoluteSrc: resolve(coreDir, name), relativeDest: `ocr/${name}` });
      }
      for (const lang of ['spa', 'eng'])
        files.push({
          absoluteSrc: resolve(root, `node_modules/@tesseract.js-data/${lang}/4.0.0_best_int/${lang}.traineddata.gz`),
          relativeDest: `ocr/${lang}.traineddata.gz`,
        });
    },
  },
  vite: () => ({
  ...
  manifest: ({ browser }) => ({
    ...
    description: 'Convert images locally and capture selected regions of webpages in many image formats.',
