      ...
      <div className="editor-workbench">
        ...
        <aside className="paint-sidebar right-sidebar" aria-label="Ajustes de edición">
          ...
          <div className="tool-group">
            ...
            <label className="field">Formato de descarga<select aria-label="Formato" value={edit.format} onChange={(event) => change({ format: event.target.value as OutputFormat })}>{OUTPUT_FORMATS.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
