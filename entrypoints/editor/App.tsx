...
export default function App() {
    ...
    const timer = setTimeout(() => {
      ...
      void saveQueue.current
        ...
        .catch((error) => {
          if (!obsolete) {
            setSaving(false);
            setStatus(`No se guardó el borrador: ${String(error)}`);
          }
        });
    }, 400);
    return () => {
  ...
  function addFiles(list: Iterable<File> | null) {
    ...
    const files = all.filter(
      (file) =>
        accepted.includes(file.name.slice(file.name.lastIndexOf('.')).toLowerCase()) || file.type.startsWith('image/'),
    );
    ...
    setStatus(
      all.length !== files.length ? `${all.length - files.length} archivo(s) no admitido(s).` : 'Imágenes agregadas.',
    );
  }
  useEffect(() => {
  ...
  async function batch(zip: boolean, retry = false) {
    ...
    const queue = retry
      ? entries.filter((entry) => results.some((result) => result.id === entry.id && !result.ok))
      : [...entries];
    ...
    try {
      for (const [index, entry] of queue.entries()) {
        ...
        try {
          ...
          if (zip) {
            if (totalBytes + encoded.bytes.length > 200 * 1024 * 1024)
              throw new Error('El ZIP supera 200 MB. Divide el lote o descarga archivos individuales.');
          ...
          report.push({
            ...
            ok: true,
            message: zip ? 'Incluida en ZIP' : 'Descarga iniciada',
          });
        } catch (error) {
          report.push({
            id: entry.id,
      ...
      setStatus(
        `${cancelled.current ? 'Lote detenido. ' : ''}${report.filter((item) => item.ok).length} completadas; ${report.filter((item) => !item.ok).length} con error.`,
      );
    } catch (error) {
      setResults(
        report.map((item) =>
          item.ok && zip ? { ...item, ok: false, message: 'No se pudo descargar el ZIP. Reintenta.' } : item,
        ),
      );
      setStatus(String(error));
    } finally {
  ...
  return (
    <main
      ...
      <header>
        <h1>Omni Image · Editor</h1>
        <p>Imágenes, capturas y texto. Todo se procesa en tu dispositivo.</p>
      </header>
      <section className="card workspace-panel">
        <div className="workspace-header">
          <div>
            <h2>Imágenes</h2>
            <p className="muted">
              {saving ? 'Guardando borrador...' : 'Borrador guardado'} · {entries.length}{' '}
              {entries.length === 1 ? 'imagen' : 'imágenes'}
            </p>
          </div>
        ...
        {drafts.some((draft) => draft.id !== sessionId) && (
          <label className="field draft-picker">
            Recuperar borrador
            <select
              value=""
              ...
              onChange={(event) => {
                if (event.target.value)
                  location.href = `${browser.runtime.getURL('/editor.html')}?session=${encodeURIComponent(event.target.value)}`;
              }}>
              <option value="">Elegir borrador local</option>
              {drafts
                .filter((draft) => draft.id !== sessionId)
                .map((draft) => (
                  <option value={draft.id} key={draft.id}>
                    {draft.name} · {draft.count} imagen(es)
                  </option>
                ))}
            </select>
          </label>
        ...
        {loadFailed && (
          <div className="toolbar">
            ...
            <button
              onClick={() => {
                location.href = `${browser.runtime.getURL('/editor.html')}?session=${crypto.randomUUID()}`;
              }}>
              Abrir borrador nuevo
            </button>
          </div>
        )}
        {!loading && !entries.length && (
          <label className="dropzone" data-dragging={dragging || undefined}>
            <input
              className="visually-hidden"
              aria-label="Agregar imágenes al arrastrar"
              type="file"
              multiple
              accept={ACCEPTED_INPUT_EXTENSIONS}
            ...
            <span className="muted">
              o haz clic para elegir archivos · también puedes pegar una captura con Ctrl/Cmd+V
            </span>
          </label>
        )}
        {!!entries.length && (
          <>
            <div className="image-list" aria-label="Imágenes del borrador">
              {entries.map((entry, index) => (
                <div key={entry.id} className={entry.id === active?.id ? 'image-tab selected' : 'image-tab'}>
                  <button
                    className="image-tab-main"
            ...
            <div className="workspace-commandbar">
              <div className="batch-actions">
                <label className="field batch-format">
                  Formato de descarga
                  <select
                    aria-label="Formato del lote"
                    value={batchFormat}
                    disabled={busy}
                    onChange={(event) => setBatchFormat(event.target.value as OutputFormat)}>
                ...
                <button className="primary" disabled={busy} onClick={() => void batch(false)}>
                  Descargar todo ({entries.length})
                </button>
                <button disabled={busy} onClick={() => void batch(true)}>
                  Descargar ZIP del lote
                </button>
                {results.some((result) => !result.ok) && (
                  <button disabled={busy} onClick={() => void batch(false, true)}>
                    Reintentar fallidas
                  </button>
                )}
                {busy && (
                  <button
                    onClick={() => {
                      ...
                      setStatus('Se detendrá al terminar la imagen actual.');
                    }}>
                    Detener lote
                  </button>
                )}
      ...
      {!!results.length && (
        <details className="card" open>
          <summary>Resultados del lote</summary>
          <ul>
            {results.map((result) => (
              <li key={result.id} className={result.ok ? '' : 'note error'}>
      ...
      {active && (
        <ImageEditorSection
          ...
          disabled={busy}
          onChange={(edit) =>
            setEntries((items) => items.map((item) => (item.id === active.id ? { ...item, edit } : item)))
          }
        />
      )}
    </main>
