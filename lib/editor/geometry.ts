...
export function cropFromAnchor(anchor: Point, p: Point, bounds: Bounds, ratio: number): CropRect {
  let width = Math.max(1, Math.abs(p.x - anchor.x)),
    height = Math.max(1, Math.abs(p.y - anchor.y));
  if (ratio) {
    height = width / ratio;
...
export function annotationBounds(item: Annotation): Box {
  if (item.kind === 'text') {
    ...
    const width = Math.min(Math.max(0.035, Math.max(...lines.map((line) => line.length), 1) * item.size * 0.5), 1);
    const height = Math.min(Math.max(0.035, lines.length * item.size * 1.2), 1);
    return { left: clamp(item.x, 0, 1 - width), top: clamp(item.y, 0, 1 - height), width, height };
  }
  if (item.kind === 'number') {
    const radius = Math.max(0.02, item.size * 0.8);
    const size = Math.min(radius * 2, 1);
    return {
      left: clamp(item.x - radius, 0, 1 - size),
      top: clamp(item.y - radius, 0, 1 - size),
      width: size,
      height: size,
    };
  }
  const left = Math.min(item.x, item.endX),
    right = Math.max(item.x, item.endX);
  const top = Math.min(item.y, item.endY),
    bottom = Math.max(item.y, item.endY);
  return { left, top, width: Math.max(0.025, right - left), height: Math.max(0.025, bottom - top) };
}
...
export function resizeAnnotation(item: Annotation, mode: string, p: Point, start: Point): Annotation {
  ...
  let left = Math.min(item.x, item.endX),
    right = Math.max(item.x, item.endX);
  let top = Math.min(item.y, item.endY),
    bottom = Math.max(item.y, item.endY);
  if (corner.includes('w')) left = clamp(left + dx, 0, right - 0.002);
  if (corner.includes('e')) right = clamp(right + dx, left + 0.002, 1);
  if (corner.includes('n')) top = clamp(top + dy, 0, bottom - 0.002);
  if (corner.includes('s')) bottom = clamp(bottom + dy, top + 0.002, 1);
...
export function textAnnotationAt(annotations: Annotation[], at: Point): number | null {
  for (let index = annotations.length - 1; index >= 0; index--) {
    ...
    if (
      at.x >= bounds.left &&
      at.x <= bounds.left + bounds.width &&
      at.y >= bounds.top &&
      at.y <= bounds.top + bounds.height
    )
      return index;
  }
  return null;
