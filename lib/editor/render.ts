...
export function drawAnnotations(
  ...
  for (const a of annotations) {
    ...
    ctx.globalAlpha = isRedaction ? 1 : Math.min(1, Math.max(0.1, a.opacity ?? 1));
    ctx.lineWidth = Math.max(2, size / 4);
    ctx.lineCap = 'round';
    const left = Math.min(x, ex),
      top = Math.min(y, ey),
      w = Math.abs(ex - x),
      h = Math.abs(ey - y);
    if (a.kind === 'rect' || a.kind === 'redact') {
      if (a.kind === 'redact') {
        ...
        ctx.fillRect(Math.floor(left), Math.floor(top), Math.ceil(w) + 1, Math.ceil(h) + 1);
      ...
      const head = Math.max(2, Math.min(size, Math.hypot(ex - x, ey - y) * 0.8));
      const halfWidth = head * 0.48;
      const baseX = ex - Math.cos(angle) * head,
        baseY = ey - Math.sin(angle) * head;
...
function drawBubble(
  ...
  const radius = Math.min(width, height, size * 0.7) / 2;
  const tail = Math.min(width * 0.22, height * 0.24, size * 1.2);
  const right = left + width,
    bottom = top + height - tail;
  ctx.beginPath();
  ...
  ctx.lineTo(left + width * 0.62, bottom);
  ctx.lineTo(left + width * 0.48, bottom + tail);
  ctx.lineTo(left + width * 0.42, bottom);
...
export async function renderEdit(source: ImageFrame, edit: EditState): Promise<ImageFrame> {
  ...
  const cropped = edit.crop
    ? cropFrame(transformed, edit.crop, formatInfo(edit.format).alpha ? undefined : edit.background)
    : transformed;
  const width = Math.round(
    edit.width || (edit.height ? (edit.height * cropped.width) / cropped.height : cropped.width),
  );
  const height = Math.round(
    edit.height || (edit.width ? (edit.width * cropped.height) / cropped.width : cropped.height),
  );
  validateDimensions(width, height);
  let output: ImageFrame;
  if (edit.placement) {
    const scale = Math.min(width / cropped.width, height / cropped.height) * edit.zoom;
    output = await placeFrame(
      cropped,
      width,
      height,
      ...
      scale,
      edit.background,
      formatInfo(edit.format).alpha,
    );
  } else
    output = width === cropped.width && height === cropped.height ? cropped : await resizeFrame(cropped, width, height);
  if (!edit.annotations.length) return output;
  const canvas = frameToCanvas(output),
