...
export async function captureFullPage(tabId: number, windowId: number): Promise<ImageFrame> {
  const invoke = async (action: 'begin' | 'step' | 'end', x = 0, y = 0, hideFixed = false) => {
    const [result] = await browser.scripting.executeScript({ target: { tabId }, func: scrollPage, args: [action, x, y, hideFixed] });
    if (!result?.result) throw new Error('No se pudo preparar la página.');
    return result.result;
  };
  try {
    const page = await invoke('begin');
    const columns = Math.ceil(page.width / page.viewportWidth), rows = Math.ceil(page.height / page.viewportHeight);
    ...
    for (let row = 0; row < rows; row++) for (let column = 0; column < columns; column++) {
      ...
      if (check.viewportWidth !== page.viewportWidth || check.viewportHeight !== page.viewportHeight || check.x !== current.x || check.y !== current.y) throw new Error('La página cambió de tamaño durante la captura.');
      ...
      try {
        if (!canvas) {
          scaleX = bitmap.width / current.innerWidth; scaleY = bitmap.height / current.innerHeight;
          const width = Math.round(page.width * scaleX), height = Math.round(page.height * scaleY);
          validateDimensions(width, height); canvas = makeCanvas(width, height);
        }
        const ctx = canvas.getContext('2d')!;
        const x = Math.round(current.x * scaleX), y = Math.round(current.y * scaleY);
    ...
    return { width: canvas.width, height: canvas.height, data: new Uint8ClampedArray(canvas.getContext('2d')!.getImageData(0, 0, canvas.width, canvas.height).data) };
  } finally { await invoke('end').catch(() => {}); }
}
...
async function scrollPage(action: 'begin' | 'step' | 'end', x: number, y: number, hideFixed: boolean) {
  type CaptureState = { x: number; y: number; style: HTMLStyleElement; hidden: { element: HTMLElement; value: string; priority: string }[]; restore: () => void; timer: number; cancelled: boolean };
  ...
  if (action === 'begin') {
    ...
    const state: CaptureState = { x: scrollX, y: scrollY, style, hidden: [], timer: 0, cancelled: false, restore: () => {} };
    const cancel = (event: KeyboardEvent) => { if (event.key === 'Escape') { state.cancelled = true; state.restore(); } };
    state.restore = () => {
      clearTimeout(state.timer);
      state.hidden.forEach(({ element, value, priority }) => value ? element.style.setProperty('visibility', value, priority) : element.style.removeProperty('visibility'));
      window.scrollTo(state.x, state.y); style.remove(); document.removeEventListener('keydown', cancel, true);
  ...
  if (action === 'step') {
    ...
    clearTimeout(state.timer); state.timer = window.setTimeout(state.restore, 15000);
    if (hideFixed && !state.hidden.length) {
      for (const element of document.querySelectorAll<HTMLElement>('body *')) {
        const position = getComputedStyle(element).position;
        if (position === 'fixed' || position === 'sticky') {
          state.hidden.push({ element, value: element.style.getPropertyValue('visibility'), priority: element.style.getPropertyPriority('visibility') });
  ...
  return {
    ...
    innerWidth: innerWidth, innerHeight: innerHeight, x: scrollX, y: scrollY,
  };
}
