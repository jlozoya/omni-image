...
export async function recognizeText(frame: ImageFrame, language: string, progress: (text: string) => void): Promise<string> {
  ...
  const worker = await createWorker(language, 1, {
    ...
    logger: ({ status, progress: value }) => progress(`${status === 'recognizing text' ? 'Reconociendo texto' : 'Preparando OCR'}: ${Math.round(value * 100)}%`),
  });
  try {
